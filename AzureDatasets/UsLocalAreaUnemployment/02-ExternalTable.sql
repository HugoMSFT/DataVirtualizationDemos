...
-- Run this script connected to the [UsLocalAreaUnemployment] database (created in 01).
-- =============================================================================

-- =============================================================================
-- Step 1: External data source (points at the dataset's storage folder)
-- =============================================================================
IF NOT EXISTS (SELECT 1 FROM sys.external_data_sources WHERE name = 'UsLocalAreaUnemploymentDS')
    CREATE EXTERNAL DATA SOURCE UsLocalAreaUnemploymentDS
    WITH (
        LOCATION = 'abs://laborstatisticscontainer@azureopendatastorage.blob.core.windows.net/laus'
    );
GO
...
IF OBJECT_ID('dbo.UsLocalAreaUnemployment_External') IS NOT NULL
    DROP EXTERNAL TABLE dbo.UsLocalAreaUnemployment_External;
GO

CREATE EXTERNAL TABLE dbo.UsLocalAreaUnemployment_External
(
...
WITH (
    DATA_SOURCE = UsLocalAreaUnemploymentDS,
    LOCATION    = '/',
...
CREATE STATISTICS ST_UsLocalAreaUnemployment_series_id ON dbo.UsLocalAreaUnemployment_External ([series_id]) WITH FULLSCAN;
CREATE STATISTICS ST_UsLocalAreaUnemployment_year ON dbo.UsLocalAreaUnemployment_External ([year]) WITH FULLSCAN;
CREATE STATISTICS ST_UsLocalAreaUnemployment_period ON dbo.UsLocalAreaUnemployment_External ([period]) WITH FULLSCAN;
GO
...
-- =============================================================================
SELECT TOP 50 *
FROM dbo.UsLocalAreaUnemployment_External;
GO
...
WHERE c.object_id = OBJECT_ID('dbo.UsLocalAreaUnemployment_External')
