...
IF DB_ID(N'UsStateEmploymentEarnings') IS NULL
    CREATE DATABASE [UsStateEmploymentEarnings];
GO

-- -----------------------------------------------------------------------------
-- IMPORTANT: Azure SQL Database does NOT support USE to switch databases.
--   Open a NEW query window connected to [UsStateEmploymentEarnings] and run everything below.
--   (On SQL Server / Managed Instance you can simply uncomment the USE line.)
-- -----------------------------------------------------------------------------
-- USE [UsStateEmploymentEarnings];
-- GO
...
SELECT COUNT(*) AS total_rows FROM dbo.UsStateEmploymentEarnings_Local;
GO

SELECT TOP 10 * FROM dbo.UsStateEmploymentEarnings_Local;
GO

-- Inspect the types SQL Server inferred from the Parquet metadata.
EXEC sp_help 'dbo.UsStateEmploymentEarnings_Local';
GO

-- =============================================================================
-- Step 3: Add indexes for better query performance
-- =============================================================================
CREATE CLUSTERED INDEX CIX_UsStateEmploymentEarnings_series_id_year_period
    ON dbo.UsStateEmploymentEarnings_Local ([series_id], [year], [period]);
GO

-- =============================================================================
-- Step 4: Analytical query - Observations by state
-- =============================================================================
SELECT TOP 20
    state_name,
    COUNT(*)                              AS observations,
    CAST(AVG(value) AS DECIMAL(18,3))     AS avg_value
FROM dbo.UsStateEmploymentEarnings_Local
GROUP BY state_name
