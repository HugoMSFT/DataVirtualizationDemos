...
-- Run this script connected to the [UsConsumerPriceIndex] database (created in 01).
-- =============================================================================

-- =============================================================================
-- Step 1: External data source (points at the dataset's storage folder)
-- =============================================================================
IF NOT EXISTS (SELECT 1 FROM sys.external_data_sources WHERE name = 'UsConsumerPriceIndexDS')
    CREATE EXTERNAL DATA SOURCE UsConsumerPriceIndexDS
    WITH (
        LOCATION = 'abs://laborstatisticscontainer@azureopendatastorage.blob.core.windows.net/cpi'
    );
GO
...
IF OBJECT_ID('dbo.UsConsumerPriceIndex_External') IS NOT NULL
    DROP EXTERNAL TABLE dbo.UsConsumerPriceIndex_External;
GO

CREATE EXTERNAL TABLE dbo.UsConsumerPriceIndex_External
(
...
WITH (
    DATA_SOURCE = UsConsumerPriceIndexDS,
    LOCATION    = '/',
...
CREATE STATISTICS ST_UsConsumerPriceIndex_series_id ON dbo.UsConsumerPriceIndex_External ([series_id]) WITH FULLSCAN;
CREATE STATISTICS ST_UsConsumerPriceIndex_year ON dbo.UsConsumerPriceIndex_External ([year]) WITH FULLSCAN;
CREATE STATISTICS ST_UsConsumerPriceIndex_period ON dbo.UsConsumerPriceIndex_External ([period]) WITH FULLSCAN;
GO
...
-- =============================================================================
SELECT TOP 50 *
FROM dbo.UsConsumerPriceIndex_External;
GO
...
WHERE c.object_id = OBJECT_ID('dbo.UsConsumerPriceIndex_External')
