...
IF DB_ID(N'UsConsumerPriceIndex') IS NULL
    CREATE DATABASE [UsConsumerPriceIndex];
GO

-- -----------------------------------------------------------------------------
-- IMPORTANT: Azure SQL Database does NOT support USE to switch databases.
--   Open a NEW query window connected to [UsConsumerPriceIndex] and run everything below.
--   (On SQL Server / Managed Instance you can simply uncomment the USE line.)
-- -----------------------------------------------------------------------------
-- USE [UsConsumerPriceIndex];
-- GO
...
SELECT COUNT(*) AS total_rows FROM dbo.UsConsumerPriceIndex_Local;
GO

SELECT TOP 10 * FROM dbo.UsConsumerPriceIndex_Local;
GO

-- Inspect the types SQL Server inferred from the Parquet metadata.
EXEC sp_help 'dbo.UsConsumerPriceIndex_Local';
GO

-- =============================================================================
-- Step 3: Add indexes for better query performance
-- =============================================================================
CREATE CLUSTERED INDEX CIX_UsConsumerPriceIndex_series_id_year_period
    ON dbo.UsConsumerPriceIndex_Local ([series_id], [year], [period]);
GO

-- =============================================================================
-- Step 4: Analytical query - Observations and average index by area
-- =============================================================================
SELECT TOP 20
    area_name,
    COUNT(*)                              AS observations,
    CAST(AVG(value) AS DECIMAL(18,3))     AS avg_value
FROM dbo.UsConsumerPriceIndex_Local
GROUP BY area_name
