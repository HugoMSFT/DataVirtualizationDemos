...
-- Run this script connected to the [UsProducerPriceIndexCommodities] database (created in 01).
-- =============================================================================

-- =============================================================================
-- Step 1: External data source (points at the dataset's storage folder)
-- =============================================================================
IF NOT EXISTS (SELECT 1 FROM sys.external_data_sources WHERE name = 'UsProducerPriceIndexCommoditiesDS')
    CREATE EXTERNAL DATA SOURCE UsProducerPriceIndexCommoditiesDS
    WITH (
        LOCATION = 'abs://laborstatisticscontainer@azureopendatastorage.blob.core.windows.net/ppi_commodity'
    );
GO
...
IF OBJECT_ID('dbo.UsProducerPriceIndexCommodities_External') IS NOT NULL
    DROP EXTERNAL TABLE dbo.UsProducerPriceIndexCommodities_External;
GO

CREATE EXTERNAL TABLE dbo.UsProducerPriceIndexCommodities_External
(
...
WITH (
    DATA_SOURCE = UsProducerPriceIndexCommoditiesDS,
    LOCATION    = '/',
...
CREATE STATISTICS ST_UsProducerPriceIndexCommodities_series_id ON dbo.UsProducerPriceIndexCommodities_External ([series_id]) WITH FULLSCAN;
CREATE STATISTICS ST_UsProducerPriceIndexCommodities_year ON dbo.UsProducerPriceIndexCommodities_External ([year]) WITH FULLSCAN;
CREATE STATISTICS ST_UsProducerPriceIndexCommodities_period ON dbo.UsProducerPriceIndexCommodities_External ([period]) WITH FULLSCAN;
GO
...
-- =============================================================================
SELECT TOP 50 *
FROM dbo.UsProducerPriceIndexCommodities_External;
GO
...
WHERE c.object_id = OBJECT_ID('dbo.UsProducerPriceIndexCommodities_External')
