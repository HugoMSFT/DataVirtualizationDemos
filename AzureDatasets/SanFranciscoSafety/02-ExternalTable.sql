...
-- Run this script connected to the [SanFranciscoSafety] database (created in 01).
-- =============================================================================

-- =============================================================================
-- Step 1: External data source (points at the dataset's storage folder)
-- =============================================================================
IF NOT EXISTS (SELECT 1 FROM sys.external_data_sources WHERE name = 'SanFranciscoSafetyDS')
    CREATE EXTERNAL DATA SOURCE SanFranciscoSafetyDS
    WITH (
        LOCATION = 'abs://citydatacontainer@azureopendatastorage.blob.core.windows.net/Safety/Release/city=SanFrancisco'
    );
GO
...
IF OBJECT_ID('dbo.SanFranciscoSafety_External') IS NOT NULL
    DROP EXTERNAL TABLE dbo.SanFranciscoSafety_External;
GO

CREATE EXTERNAL TABLE dbo.SanFranciscoSafety_External
(
    [dataType]           VARCHAR(50),
...
WITH (
    DATA_SOURCE = SanFranciscoSafetyDS,
    LOCATION    = '/',
...
CREATE STATISTICS ST_SanFranciscoSafety_dateTime ON dbo.SanFranciscoSafety_External ([dateTime]) WITH FULLSCAN;
CREATE STATISTICS ST_SanFranciscoSafety_category ON dbo.SanFranciscoSafety_External ([category]) WITH FULLSCAN;
GO
...
-- =============================================================================
SELECT TOP 50 *
FROM dbo.SanFranciscoSafety_External
...
WHERE c.object_id = OBJECT_ID('dbo.SanFranciscoSafety_External')
