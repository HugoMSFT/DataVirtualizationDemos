...
-- Run this script connected to the [ChicagoSafety] database (created in 01).
-- =============================================================================

-- =============================================================================
-- Step 1: External data source (points at the dataset's storage folder)
-- =============================================================================
IF NOT EXISTS (SELECT 1 FROM sys.external_data_sources WHERE name = 'ChicagoSafetyDS')
    CREATE EXTERNAL DATA SOURCE ChicagoSafetyDS
    WITH (
        LOCATION = 'abs://citydatacontainer@azureopendatastorage.blob.core.windows.net/Safety/Release/city=Chicago'
    );
GO
...
IF OBJECT_ID('dbo.ChicagoSafety_External') IS NOT NULL
    DROP EXTERNAL TABLE dbo.ChicagoSafety_External;
GO

CREATE EXTERNAL TABLE dbo.ChicagoSafety_External
(
    [dataType]           VARCHAR(50),
...
CREATE STATISTICS ST_ChicagoSafety_dateTime ON dbo.ChicagoSafety_External ([dateTime]) WITH FULLSCAN;
CREATE STATISTICS ST_ChicagoSafety_category ON dbo.ChicagoSafety_External ([category]) WITH FULLSCAN;
GO
...
-- =============================================================================
SELECT TOP 50 *
FROM dbo.ChicagoSafety_External
...
WHERE c.object_id = OBJECT_ID('dbo.ChicagoSafety_External')
