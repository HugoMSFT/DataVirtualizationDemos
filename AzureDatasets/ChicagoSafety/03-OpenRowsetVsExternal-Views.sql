...
--         * dbo.vw_ChicagoSafety_OpenRowset  -> OPENROWSET (ad-hoc, schema via WITH)
--         * dbo.vw_ChicagoSafety_External    -> the external table from script 02
-- Prerequisite: run 02-ExternalTable.sql first (creates dbo.ChicagoSafety_External).
-- Run this script connected to the [ChicagoSafety] database.
-- =============================================================================

-- =============================================================================
-- Step 1: View over OPENROWSET
-- =============================================================================
CREATE OR ALTER VIEW dbo.vw_ChicagoSafety_OpenRowset
AS
SELECT
    [dataType],
    [dataSubtype],
...
-- =============================================================================
CREATE OR ALTER VIEW dbo.vw_ChicagoSafety_External
AS
SELECT * FROM dbo.ChicagoSafety_External;
GO

-- =============================================================================
-- Step 3: Same shape, two engines - row counts should match
-- =============================================================================
SELECT COUNT(*) AS rows_via_openrowset FROM dbo.vw_ChicagoSafety_OpenRowset;
GO
SELECT COUNT(*) AS rows_via_external   FROM dbo.vw_ChicagoSafety_External;
GO
...
FROM dbo.vw_ChicagoSafety_OpenRowset
GROUP BY category
...
-- External-table view
SELECT category, COUNT(*) AS incident_count
FROM dbo.vw_ChicagoSafety_External
GROUP BY category
ORDER BY incident_count DESC;
GO
...
-- Cleanup (optional)
-- =============================================================================
-- DROP VIEW IF EXISTS dbo.vw_ChicagoSafety_OpenRowset;
-- DROP VIEW IF EXISTS dbo.vw_ChicagoSafety_External;
-- GO
